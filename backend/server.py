...
# 配置常量
DEBUG_MODE = os.getenv('DebugMode', 'true').lower() == 'true'  # 本地开发默认开启调试
SHOW_VCP_OUTPUT = os.getenv('ShowVCP', 'true').lower() == 'true'  # 本地开发默认显示 VCP
PORT = int(os.getenv('PORT', 6005))
API_KEY = os.getenv('API_Key')
API_URL = os.getenv('API_URL')
SERVER_KEY = os.getenv('Key', 'local-dev')  # 本地开发默认值
...
# ==================== 认证中间件 ====================
# 本地开发模式：已禁用认证，如需启用请取消注释
#
# @app.middleware("http")
# async def auth_middleware(request: Request, call_next):
#     """认证中间件"""
#     # 图片服务路径豁免
#     if re.match(r'^/pw=[^/]+/images/', request.url.path):
#         return await call_next(request)
#
#     # 检查 Bearer token
#     auth_header = request.headers.get('authorization')
#     if auth_header != f'Bearer {SERVER_KEY}':
#         return JSONResponse(
#             status_code=401,
